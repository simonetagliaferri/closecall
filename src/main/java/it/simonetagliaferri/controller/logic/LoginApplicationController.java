...
public class LoginApplicationController extends ApplicationController {

    private final LoginDAO loginDAO;
    private final HostDAO hostDAO;
    private final PlayerDAO playerDAO;

    public LoginApplicationController(SessionManager sessionManager, LoginDAO loginDAO, HostDAO hostDAO, PlayerDAO playerDAO) {
    ...
    */
    public void signup(UserBean bean) {
        ...
        user.hashPassword();
        if (loginDAO.findByUsername(user.getUsername()) != null || loginDAO.findByEmail(user.getEmail()) != null) {
            throw new DuplicateUserException("Username or email already exist, something went wrong during signup.");
        }
