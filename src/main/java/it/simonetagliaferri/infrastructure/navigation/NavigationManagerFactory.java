...
public class NavigationManagerFactory {
    ...
    public NavigationManager getNavigationManager(AppContext context, UIMode uiMode) {
        return getNavManager(context, uiMode);
    }

    private NavigationManager getNavManager(AppContext context, UIMode uiMode) {
        switch (uiMode) {
            case CLI:
